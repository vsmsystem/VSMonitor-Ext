...
document.addEventListener('change', function(e){
	if(e.target.getAttribute("name") == "perfil"){
			...
			localStorage['rf2_estado']=document.getElementsByName('perfil')[0].options[document.getElementsByName('perfil')[0].options.selectedIndex].value.split('_')[2]
			console.log(localStorage['rf2_estado'])
...
var $wrapper = document.querySelector('.purple'),
	...
	+'<div id="aviso" style="clear:both;"> &nbsp; </div>'
...
var texto = document.getElementById('manobraunicabox').value.replace(/(?:\r\n|\r|\n|\t)/g, '\n');

function irsozinho() {
	var texto = document.getElementById('manobraunicabox').value.replace(/(?:\r\n|\r|\n|\t)/g, '\n');
	if ((texto.length>0)&(texto.indexOf('tecle Enter para buscar')>-1)&(texto.indexOf('Pesquisar por:')<0)){
	//console.log("Deu certo");
		...
		if(texto.indexOf('Rin')>-1){
			...
			var contagem = contagem.split ('Porta')[1];
			...
			var contagemA = contagem[0];
			var contagemB = contagem[1];
			if(contagem.indexOf('IMS')>-1){
			var contagemF = contagemA+contagemB;
			}else{
			var contagemF = contagemA+'-'+contagemB;
			}
			var contagemF = contagemF.replace(/(?:COMBO|MIX|IMS|\n)/g, '');			
		}
		...
		setTimeout(function (){
			...
			if ((cidadeOrigem==0|cidadeOrigem==null)&sessionStorage.getItem('setCidade')!='ok'){
				...
				for (l=0;l<document.getElementsByName('shelfOrigem.equipmentId')[0].options.length;l++){
					if ((document.getElementsByName('shelfOrigem.equipmentId')[0].options[l].text.indexOf(sessionStorage.getItem('rin'))> -1)|(document.getElementsByName('shelfOrigem.equipmentId')[0].options[l].text.indexOf(sessionStorage.getItem('contagem'))> -1)){
		...
		//console.log("Deu certo");
		statusCopiar('<span style="display:inline-block;text-align:center;color:#ff0000;border:solid 1px #ff0000;padding:2px;width:200px;">Manobra unica nao reconhecido<br>Atencao copie e cole<br>do Google Chrome </span>') ;
 ...
 function mostrarTecnologia(){
	 ...
	 loadJSON(function(response) {
	  // Parse JSON string into object
		...
		if(document.getElementsByName('armarioDestino.locationId')[0].selectedIndex>-1){
...
elemento.addEventListener('keydown', function(e) {
    if ((e.metaKey || e.ctrlKey) && ( e.keyCode==192) ) {
        document.getElementsByName('cidadeOrigem.cityName')[0].options.selectedIndex=document.getElementsByName('cidadeOrigem.cityName')[0].options.selectedIndex+1
		javascript:VerificaCidade('Cidade');
    }
}, false);

if(document.getElementsByName('perfil')[0]){


}else{
	var totalcidades=document.getElementsByName('cidadeOrigem.cityName')[0].options.length
	var totalarmarios=document.getElementsByName('armarioOrigem.locationId')[0].options.length
	var cidadeselecionada = document.getElementsByName('cidadeOrigem.cityName')[0].options[document.getElementsByName('cidadeOrigem.cityName')[0].options.selectedIndex].value
	var estado = localStorage['rf2_estado']
	console.log('Cidade: '
	+document.getElementsByName('cidadeOrigem.cityName')[0].options[document.getElementsByName('cidadeOrigem.cityName')[0].options.selectedIndex].innerText
	+' [ '+document.getElementsByName('cidadeOrigem.cityName')[0].options.selectedIndex+' / '+(totalcidades-1)+' ]'
	+' Armarios  [ '+(totalarmarios-1)+' ]'

	)


	var loopards='';
	for (ctr=1;ctr<totalarmarios;ctr++){
		//console.log(document.getElementsByName('cidadeOrigem.cityName')[0].options[ctr].innerText)
		//cidades.push(document.getElementsByName('cidadeOrigem.cityName')[0].options[ctr].innerText)
		var loopards=loopards
		+estado
		+' ; '+cidadeselecionada
		+' ; '+document.getElementsByName('armarioOrigem.locationId')[0].options[ctr].value
		+' ; '+document.getElementsByName('armarioOrigem.locationId')[0].options[ctr].innerText
		+'\n'
	}
	console.log(loopards)
	if(totalarmarios>0){
		localStorage['banco']=localStorage['banco']+loopards;
		console.log('ards gravados')
	}
	var cidades=[]
	for (ctr=1;ctr<totalcidades;ctr++){
		//console.log(document.getElementsByName('cidadeOrigem.cityName')[0].options[ctr].innerText)
		//cidades.push(document.getElementsByName('cidadeOrigem.cityName')[0].options[ctr].innerText)
	}
